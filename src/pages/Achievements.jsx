  ...
  const checkAndReplaceAchievement = (achievementId) => {
    setAchievements((prev) => {
      ...
      const newAchievement = allAchievements.find(
        (a) => a.category === completedAchievement.category && !prev.some((p) => p.id === a.id)
      );

      let updatedAchievements;
      if (newAchievement) {
        updatedAchievements = prev.map((a) => (a.id === achievementId ? newAchievement : a));
      } else {
    ...
    <div className="min-h-screen bg-gray-900 pb-16 p-4">
      ...
      <div className="grid gap-4">
        {achievements.map((achievement) => (
          <div
            key={achievement.id}
            className={`bg-gray-800 rounded-lg p-4 ${achievement.completed ? "opacity-50" : ""}`}
            ...
            <div className="flex items-start">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  ...
                  {achievement.completed ? <span className="text-purple-500 text-sm">+{achievement.xpReward} XP</span> : null}
                </div>
